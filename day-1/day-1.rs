...
fn main() {
    ...
    'outer: for index in 0..entries.len()-2 {
        'innter: for inner_index in index+1..entries.len()-1 {
            if entries[index] + entries[inner_index] == 2020 {
                let multiples: i32 = entries[index]*entries[inner_index];
                println!("Found a pair: {},{}",entries[index],entries[inner_index]);
                println!("{}",multiples);
                return();
            }
        }
